...
- **FCM Web Push + Pusher presence** are in scope for this pass (see [05-realtime.md](05-realtime.md)); in-browser file editor + versioning and containerized execution remain out of scope.
...
| Realtime transport | **Pusher JS** (`pusher-js`) for private notification channels + presence "is reviewing"; **FCM** (`firebase`) for foreground/background device push | Explicit FE integration guide (2026-07-16). STOMP/SockJS (BE §8) remains the documented alternate in [05](05-realtime.md) if the team switches back. |
